...
function MusicPlayer(musicData) {
  ...
  var playlist = _toConsumableArray(musicData);

  var isPlaying = false;
  var audio = new Audio(musicData[trackIndex].url);
  ...
  var shuffle = document.createElement('i');
  ...
  function loadTrack(index) {
    ...
    artist.textContent = "by ".concat(musicData[trackIndex].artist);
    audio.currentTime = 0;
    progressBar.value = 0;
  ...
  function shufflePlaylist() {
    for (var i = playlist.length - 1; i > 0; i--) {
      ...
      var _ref = [playlist[j], playlist[i]];
      playlist[i] = _ref[0];
      playlist[j] = _ref[1];
    }
  }
